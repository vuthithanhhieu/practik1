...
k =5
sim_uv=dict(sorted(sim_uv.items(), key=lambda x: x[1], reverse=True)[:k])#5 пользователей которые наиболее похожи
#на нашего заданного пользователя и просто сортируем словарь по убыванию метрик и берем первые 5 записей
